...
## 【最重要】AI操作の優先順位（2026/04/27 改定）

**今後すべての業務で、API経由のAI（Gemini API・ChatGPT API等）よりも Claude in Chrome を優先する。**

### 優先順位（必ず上から検討）
1. **Claude in Chrome** （`mcp__Claude_in_Chrome__*` ツール群）
   - WordPress管理画面・カスタマイザー操作
   - Google Drive / Sheets / Apps Script 操作
   - メルカリ・X・Instagram・YouTube等のWebサービス操作
   - SaaS管理画面・スクレイピング・情報収集
   - **「人間がブラウザでクリックできる作業」は全部これ一択**
2. **MCPサーバー経由API**（Google Calendar MCP等の信頼できるもの）
3. **APIキー直接呼び出し**（Gemini API等）— 最終手段

### Claude in Chrome を優先する理由（CPO判断）
- **コスト**: API利用料ゼロ、月額数千円〜数万円の節約
- **統一性**: 全AI操作がClaudeに集約 → 学習・改善サイクルが一貫
- **デバッグ容易**: 何が起きているか目視できる
- **権限**: 既存ログインセッション・OAuth・Cookieを再利用
- **柔軟性**: UI変化に追従しやすい（コード書き直し不要）

### 既存実装の扱い
- 動いているAPI実装は**急いで置換しない**（ROI判断）
- 新規・大幅改修のタイミングで Claude in Chrome に寄せる
- 「Gemini API使用中」とある既存メモは現状維持OK

### Claude in Chrome ができないこと（API継続）
- Cloudflare Workers内などサーバー環境のバックグラウンド処理
- ユーザーが画面を見ていない時間帯の定期実行（cron的）
- ブラウザを起動できない環境
...
### 自動検出
- Stop hook が毎ターン後に `tools/session_health.py --quiet` を実行
- 警告閾値超過時のみ画面に通知される
