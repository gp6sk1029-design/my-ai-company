...
### 現在の運営と完了判定（2026-09-05）
- 着手時は `operations/CURRENT.md` を読む。対応中は全社3件まで。現在の状態・担当・次の確認日はここを正本とし、MEMORY.mdの過去TODOから勝手に作業を再開しない。
- 成果の計測・完了判定・保存と採用の手順は `operations/README.md`。未測定をゼロと扱わず、下書き保存と公開、テスト成功と実操作確認を区別する。
- 既存記事の本文の正本はWordPress。ローカルmdからの既存記事更新には、現在の本番スナップショットとmdを確認した承認記録が必要。
- ローカル保存とGitHubへの採用を分離。終了フックでは検査のみ行い、自動コミット・pushは禁止。旧履歴のauto-sync手順は現行の指示ではない。
- 部門のMEMORY.mdは証拠・学びの履歴として保持する。現在の優先順位はCURRENT.mdへリンクし、複製して管理しない。
...
現在ユーザーと対話しているAIが司令塔となり、必要なときに相手のAIへ助言や検証を依頼する。使用モデルはユーザーが今回選んだものだけ。**使うのは任意**で、呼んだ側・呼ばれた側それぞれの契約利用枠を消費する。
...
- 正規窓口は `python3 tools/subscription_ai.py <codex|claude> <check|ask|review|implement> --model <選択モデル>`。直接CLIや旧MCPで迂回しない。認証だけの `check` はモデル指定不要。
- **モデルは依頼ごとにユーザーの選択を確認する。未指定なら、相手のAIと用途を示して「どのモデルにしますか？」と会話で質問し、回答を待つ。** 既定設定・別タスクの選択を勝手に流用しない。今回の依頼ですでに明示されていれば再確認不要。同じ依頼の複数往復では選択を維持する。
- 未指定で共通窓口を呼ぶと終了コード3・「モデル選択待ち」で止まり、認証確認やAI生成は行わない。自動でモデルを補って再試行しない。Claudeは既存ルールに従いOpus系から選択を確認する。
...
- 指示・ルール類の読み込みにも利用枠を使うが、消費量はモデル・入力・設定で変わる。過去の1回の測定値を固定費と断定しない。第二意見が必要な場合だけ呼び、同じ調査を重複させない。
- `/codex-*` は共通窓口を使用し、モデルはユーザーの明示指定のみ、推論設定は現在のCodex設定を継承する。認証を迂回できる任意のCLIオプションは受け付けない。
- もっと安くしたい個別用途では、手動で軽量モデル（gpt-5.4-mini）を選ぶ手もある（下記）。

### 手動でモデルを選ぶ方法（ユーザー向け）
単独でCodexを使う場合と、共通窓口から相手を呼ぶ場合のモデル選択は別：
- **Codexアプリ**：画面のモデル選択UIで切り替え
- **ターミナル**：`codex --model <名> "…"`
- 連携では `--model` を必須にする。利用可能なモデルはその時点のCLI/アカウントで確認し、古い一覧から推測しない。
...
3. **Git運用**
   - 作業開始時：mainかつローカル変更がない場合だけ `git pull --ff-only`。競合や分岐時は停止し、自動マージしない。
   - 作業終了時：`python3 tools/operations_check.py` の検査のみ。自動コミット・pushはしない。採用は `operations/README.md` の手順に従う。
...
### 戦術4：実際の継続不能への対応
- 容量だけでは停止しない。上記「基本方針：製品標準の自動圧縮に任せる」に従い、圧縮失敗・明示依頼・役割変更時だけ引き継ぎを判断する。
- 現在の進捗はoperations/CURRENT.md、学びは部門MEMORY.mdへ記録する。
