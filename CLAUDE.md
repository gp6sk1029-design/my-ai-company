...
## 【必須】Codex連携・Obsidianビューア（2026-06-16 制定）

### このリポジトリは「複数AI共有の外部脳」
- このフォルダ（`my-ai-company`）全体が、セッションを跨いで知識を蓄える「外部脳」。中身はすべてプレーンなマークダウン＋Git。
- **Obsidian**：このフォルダを「Vault」として開くと、同じファイルを地図・グラフ・全文検索で閲覧できる**人間用ビューア**。ファイルは移動・変換しない。入口は `🏠 HOME.md`。
  - ⚠️ Obsidianはあくまで閲覧・ナビ用。知識の正本は各 `SKILL.md` / `MEMORY.md` / `reports/` / `handover/`。Obsidian用に `Knowledge/` 等の別フォルダを新設して脳を二分割しないこと。
  - `.obsidian/` の volatile ファイル（workspace.json・cache等）は `.gitignore` 済み。安定設定はコミットして2台目PCへ配る。

### Codex（gpt-5.5）との情報共有
- **Codexも同じリポジトリを共有**し、`git pull`/`push`（`.codex/hooks.json` で自動）を通じてClaudeと知識をやり取りする。
- Codexは作業前に **`AGENTS.md`（薄い案内役）→ `CLAUDE.md`（正本）** を読む。ルールはCLAUDE.md1本に集約し二重管理しない。
- `.claude/settings.json` と `.codex/hooks.json` の Stop フックは**両方に競合マーカー検査**を入れてある（壊れたファイルをpushしない最後の砦）。片方だけ直して放置しないこと。

### 🔴 ClaudeとCodexの共存ルール（競合事故の予防）
- ClaudeもCodexも全ファイルを編集してよい（役割固定はしない）。
- **同時刻に同じ作業で並走させない（順番に使う）**。開始時の `git pull` と終了時の競合マーカー検査により、逐次運用なら競合事故を防げる。
- AGENTS.md を変更したら、CLAUDE.mdとの整合（正本はCLAUDE.md）を保つこと。
