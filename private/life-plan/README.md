...
- **マネーフォワードCSV取込** — 資産残高を差分プレビュー後に一括反映（MF公開API不在のため現実解としてCSV方式）
...
├── functions/api/
│   ├── sync.js            — D1同期
│   └── mf-import.js       — MF CSVパース（サーバ側スタブ）
├── wrangler.toml
└── schema.sql             — D1テーブル定義
```
...
4. 「資産」タブで口座（新NISA・特定・株・暗号・現金）を登録、またはMF CSVを取込
