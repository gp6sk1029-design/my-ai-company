...
- 自動検査：本文X換算140字以内（URLはt.co換算23字）／リンク位置は`link_strategy`に準拠／タグはキャンペーン指定上限以内／長辺1800px以内
- リンク方針：ブログ流入を優先する場合は`link_strategy: "main"`で本文へURLを掲載。認知・X内完結を優先する場合は`link_strategy: "reply"`で返信へ分離する
- 商用関係：キャンペーンJSONへ `commercial_relation`（`self_paid / affiliate_only / provided / sponsored / own_product`）を必ず指定する。`affiliate_only` はX本文にアフィリエイト表示、`provided` は提供元、`sponsored` は本文1行目にPR＋提供元を入れる
- 安全策：既存画像は上書きしない
