...
### Step 6【ルール】: プレビューで丸をつけられた画像を直す（2026-07-20制定）

ユーザーがローカルプレビュー（`http://localhost:8794/preview/<slug>`）のスクリーンショットに**赤丸などの手書き注釈**を付けて「映えるようにして」「これ直して」と言ってきたときの標準手順。

**① 対象画像を特定する（推測しない）**
1. 注釈が囲んでいる画像の**すぐ下のキャプション文言**を読む
2. `blog/articles/<slug>.md` を `grep -n '!\['` して、そのキャプションと一致する行を探す → ファイル名確定
3. `blog/articles/<slug>_context.md` でファイル名から **Drive fileId** を引く
4. 🔴 **確定したファイル名をユーザーに一言で復唱してから着手する**（別画像を直す事故の防止）

**② 「映えない」の原因を分類してから手を動かす**

| 症状 | 対処 | 使う道具 |
|---|---|---|
| 背景に私物・生活感が写り込んでいる | 記事めしの【商品説明】プロンプトで**背景整理つき**に再生成 | 記事めしPWA（ChatGPT/Gemini） |
| 構図は良いが素っ気ない（注釈・見出しが無い） | Canvaテンプレに流し込んで見出し＋注釈を載せる | **Canva API**（下記③） |
| 商品の質感がAI生成で実物と違う | **採用しない**。実写に差し替える | — |
| 単に暗い・傾いている | Canvaで手動補正 | 分割ビュー（Step 1〜3） |

**③ Canva API で仕上げる場合の鉄則（🔴 最重要の前提）**

Canvaの `upload-asset-from-url` は **「すでに公開されているHTTPS URL」からしか画像を読み込めない**。以下は**2026-07-20に実測で確認済み**の事実：

- ❌ Google Driveの共有URL（`drive.google.com/thumbnail?id=...`）→ **HTTP 302・0バイト**。非公開なのでCanvaから取得不可
- ❌ 記事めしGASの `action=downloadFile` → **JSON(base64)を返す**ので画像URLとして使えない
- ❌ **一時アップロードサービス（ギガファイル便・imgur・pastebin等）は使用禁止**。幸田さんの写真が公開ネットに残り、取り消せない
- ❌ **GASのURLにトークンを付けてCanvaに渡すのも禁止**。記事めしAPI全体の鍵が外部サービスのログに残る

✅ **唯一の正しい橋渡し＝WordPressメディアライブラリ**
その画像は記事公開時にどのみちWPへ上がる。順番が前後するだけなので、これが最も筋が通る。

```
① WPメディアへ先行アップロード → 公開URLを得る
② Canva upload-asset-from-url にそのURLを渡す
③ 枠デザイン(DAHPxq8x0v8)を copy-design で複製
④ perform-editing-operations の update_fill で画像を流し込み、
   replace_text で見出し・注釈を差し替え → commit-editing-transaction
⑤ export-design でPNG書き出し
⑥ canva_to_meshi.py で記事めしの同じ記事フォルダへ戻す
⑦ 仕上げ後に元の実写が不要なら、WPメディアから削除する
```

🔴 **①を実行する前に必ずユーザーの承認を取る**（画像が公開URLを持つため）。承認なしでWPへ上げない。

**④ Canva APIでできること／できないこと（2026-07-20実測）**

| 操作 | 可否 | 備考 |
|---|---|---|
| 既存の画像枠に画像を流し込む（`update_fill`） | ✅ | テンプレに枠が必要 |
| 画像を新規挿入（`insert_fill`） | ✅ | 位置・サイズ指定可 |
| 既存テキストの差し替え（`replace_text`） | ✅ | テンプレに文字要素が必要 |
| **テキスト要素をゼロから新規追加** | ❌ | **できない** → 必ずテンプレ側に用意しておく |
| **空のテキスト要素に文字を入れる** | ❌ | 🔴 `replace_text` が `internal_error` で失敗（**2026-07-20実測**）。中身が空（`regions: []`）の枠は**APIから埋められない**。テンプレには必ず**ダミー文字を入れておく**こと（例:「ラベル1」）。空枠は「あとで埋められる箱」ではなく**死んだ枠** |
| 既存要素の移動（`position_element`） | ✅ | top/left指定 |
| 既存要素の拡大縮小（`resize_element`） | ✅ | アスペクト比保持オプションあり |
| 既存要素の**回転** | ❌ | 挿入時(`insert_fill`)にしか角度指定できない |
| フォントファミリの変更 | ❌ | サイズ・太さ・色・下線・行間は可 |

**🔺 矢印・囲みは「図形を描く命令」は無いが、実質3つの方法で入れられる**（2026-07-20訂正。当初「不可」と誤記した）

| 方法 | 位置 | サイズ | 回転 | 前提・向き不向き |
|---|---|---|---|---|
| **① 枠デザインに図形を仕込んでおく**<br>→ `position_element` / `resize_element` で動かす | ✅ | ✅ | ❌ | **最も確実**。よく使う矢印・角丸囲み・番号バッジを枠に置いておく。回転できないので**上下左右4方向の矢印を最初から用意**しておくと詰まらない |
| **② 矢印を画像アセットとして挿入**<br>→ `insert_fill`（`asset_type: "image"`） | ✅ | ✅ | **✅**<br>(-180〜180°) | 角度を自由に付けられる唯一の方法。ただし**矢印PNG自体も公開URLが必要**（透過PNGを一度WPメディアに置いて使い回すのが現実的） |
| **③ `generate-design` にお任せ** | AI任せ | AI任せ | AI任せ | `asset_ids` に写真を渡すとAIが図形入りで生成。**制御性が低い**ので一発勝負向き |

**⑤ `generate-design` を注釈画像の生成に使うときの注意（2026-07-20実測）**

写真アセットを渡して「見出し＋ラベル＋矢印」を指示すると、**レイアウト（写真配置・オレンジの矢印・白いラベル枠）は高品質に出る**が、**ラベルの中身の文字が空のまま生成されることが多い**（4案中3案が空枠、1案は文言を勝手に言い換え）。そして上表のとおり**空枠はAPIから埋められない**ため、そのままでは完成しない。

→ 対処：`generate-design` は**「テンプレの雛形を1回だけ作る」用途に限定**する。生成物はCanva画面で**手動でダミー文字を入れてから**正式テンプレとして保存する。以降はそのテンプレを `copy-design` → `replace_text` で使い回せば、2枚目以降は完全にAPIで回る。

→ 結論：**「決まった型の注釈」ならAPIで回せる**（①＋②を仕込んだ枠デザインを育てるのが本筋）。一方、**毎回違う場所を指す一点物の注釈**は、枠づくりの手間がリターンを上回るので Step 1〜3 の分割ビュー（手動）か記事めしの【商品説明】テンプレが速い。**ROIで選ぶこと。**
...
- 🔴 **画像をCanvaに渡すために、一時アップロードサービス（ギガファイル便・imgur・pastebin等）へ上げない**。取り消せない公開になる。公開URLが要るときは**WPメディア経由＋事前承認**のみ（Step 6-③）
- 🔴 **GASのURL（トークン付き）を外部サービスに渡さない**。記事めしAPI全体の鍵が漏れる
