...
(() => {
  ...
  // ─── AI 接続先設定（端末固有値はlocalStorage、共有値はGoogle Driveへ同期） ────────
  ...
  const CONN_KEY_CODEX_SNS_THREAD = 'pwa-meshi-conn-codex-sns-thread';
  ...
  async function saveSharedAIConnections(chatgptUrl, geminiUrl, codexSnsThreadId) {
    const body = new URLSearchParams({
      token: TOKEN,
      action: 'saveAIConnections',
      settingsJson: JSON.stringify({ chatgptUrl, geminiUrl, codexSnsThreadId }),
    });
  ...
  setTimeout(() => {
    ...
    loadSharedAIConnections().then((data) => {
      ...
      const snsThreadId = (data.settings && data.settings.codexSnsThreadId) || DEFAULT_CODEX_SNS_THREAD_ID;
    ...
    if (btnReset) btnReset.addEventListener('click', async () => {
      ...
      localStorage.removeItem(CONN_KEY_CODEX_SNS_THREAD);
  ...
  const TEMPLATE_FIELDS = {
    ...
    product_desc: {
      title: ['見出し（この画像で説明すること）', '例：小さいのに全部入る理由'],
      main:  ['注釈（部分＝説明・/区切り・2〜4個）', '例：L字ファスナー＝片手で一気に開く / 内側の仕切り＝小銭とカードを分けて収納 / 表面＝防水ナイロン生地'],
      sub:   ['強調・図形の指示（任意）', '例：薄い部分を赤い囲みで強調 / 開き口に矢印'],
      mood:  ['配色',         '例：写真そのまま＋青見出し＋オレンジ矢印'],
    },
    oshi: {
  ...
  function buildResearchPrompt(key, vars) {
    ...
    const M = {
      ...
      product_desc: `上記の製品について、写真に注釈（矢印・図形・ラベル）で指し示したい「注目ポイント」を教えてください。※この1枚で全部を説明しきる必要はありません。要点だけ：\n- 見出し（この画像で伝えること・1つ・15文字前後）\n- 注釈 2〜4個（「部分名＝短い説明」の形で・各20文字以内・"/"区切りで）\n- 特に矢印や囲みで強調すべき箇所（あれば1つ）\n公式情報と実機レビューを根拠に、誇張なく。`,
...
${COMMON_GUARDS}`,
    ...
    // 🖊️ 商品説明（実機写真に注釈・矢印・図形を重ねる説明図・説明の"1枚"）
    product_desc: ({title, main, sub, mood}) => `# 注釈つき説明画像（実機写真に矢印・図形・ラベルで指し示す）

【出力仕様】1200×630px / PNG

【最重要・写真の扱い】
- 添付画像がある場合：写っている**商品そのものは一切変えない**（形状・色・ロゴ・質感・向き・素材感を保持。商品を描き直さない・別物に置き換えない）。その上に注釈（ラベル・矢印・図形）を重ねる。
- 🚨 **背景だけは整理して無駄なものを消す**：商品と無関係な写り込みは全て取り除く。具体的には——散らかった小物・生活感のある雑多な物・手や人物・柄の強い床/机/布・他社製品や文字入りパッケージ・ケーブル類・反射で写り込んだ室内・雑然とした影。
- 整理後の背景は**無地に近い清潔な面**にする（白 #ffffff 〜 ごく淡いグレー #f8fafc）。商品の**接地影だけは自然に残す**（浮いて見せない）。
- 🚫 背景に**情報を足さない**：新しい小物・装飾・模様・ロゴ・文字・グラデーション・光条などを勝手に追加しない。「消して無地にする」だけ。
- 理由：背景が静かなほど、注釈のラベル・矢印・囲みが読みやすく、指し示したい部分に視線が集中する。
- 添付画像がない場合のみ、商品を写実的に描いてから注釈を重ねる（この場合も背景は無地）。

【この画像の役割】
- これは商品説明の"1枚"。ここで全部を説明しきる必要はない。指定された注目ポイントだけを、注釈で分かりやすく指し示す（詰め込みすぎない）。

【見出し】
- 上部に見出し（22-28pt・青 #1d4ed8 極太・写真を隠さない位置）：「${title || 'この画像で説明すること'}」

【注釈（ラベル＋引き出し線／矢印）】
- 対象：${main || 'L字ファスナー＝片手で一気に開く / 内側の仕切り＝小銭とカードを分けて収納 / 表面＝防水ナイロン生地'}
- "/"で区切られた各項目を1つの注釈にする（項目数は入力どおり・勝手に増減しない）
- 各注釈：該当部分から細い引き出し線 or 矢印（オレンジ #f97316・線1.5〜2px・矢先を明確に）を引き、白背景の角丸ラベル（枠 #e5e7eb・影うすく）に「部分名＝説明」を14-18ptで配置。「＝」の前（部分名）は黒 #111827 太字、後ろ（説明）は濃グレー #374151
- 矢印・ラベルは対象部分に正確に対応させ、写真の主役を隠しすぎない

【強調図形（任意）】
- 注目させたい箇所は図形で囲う：円 or 角丸長方形の枠線（オレンジ #f97316・2px・塗りは無し or 半透明）
- 追加指示があれば従う：${sub || '（特に強調したい箇所・円や囲みで目立たせたい部分があれば指定）'}

【配色】
${mood || '商品はそのまま・背景は無地(白〜#f8fafc) / 見出し青 #1d4ed8 / 矢印・強調図形オレンジ #f97316 / ラベルは白背景＋黒文字 #111827'}

【スタイル】家電・ガジェットの取説／レビュー記事の「ここがポイント」注釈図の一級品。線・矢印・ラベルは細く整い、指定ポイントのみを的確に指し示す。
${COMMON_GUARDS}`,
...
${COMMON_GUARDS}`,
  ...
  const TEMPLATE_PLAN = {
    ...
    product_desc: { rank: 44, section: '本文（注釈つき説明）',      caption: '写真に注釈・矢印・図形で指し示す説明（複数可）' },
