...
記事めしの「🧠 SNS統括PDMで画像生成」は、設定済みの既存SNS統括PDMタスクを開く。クリップボードへコピーするのは画像ジョブの実行依頼だけで、セッション宣言・役割定義・引き継ぎ指示は混ぜない。編集元画像は`.article-meshi-codex`へ保存し、依頼文に絶対パスとMarkdown画像参照を含めるため、Codexが`view_image`で読み込める。チャットへ画像自体を貼りたい場合だけ「🖼 編集元画像をコピー」を使う。SNS統括PDMを新しいセッションへ更新したときは、「AI 接続先設定」の「SNS統括PDMの移動先」を新しいタスクIDへ替えて保存する。

## PC・スマホ間の設定同期

「AI 接続先設定」で保存したChatGPTプロジェクトURL、Gemini Gem URL、SNS統括PDMのタスクIDは、Google Driveの`記事めし_AI接続先設定.json`へ保存する。記事めしを開くたびにこのファイルを読み込むため、PCとスマホで同じ設定を共有できる。
