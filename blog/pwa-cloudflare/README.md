...
記事めしの「🧠 SNS統括PDMで画像生成」は、設定済みの既存SNS統括PDMタスクを開く。画像生成の指示はクリップボードへコピーされるので、開いたSNS統括PDMへ貼り付けて送信する。SNS統括PDMを新しいセッションへ更新したときは、「AI 接続先設定」の「SNS統括PDMの移動先」を新しいタスクIDへ替えて保存する。
