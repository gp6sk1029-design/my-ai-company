...
3枚以上の新規画像を「すべて転送」すると、高速一括転送を自動で使う。GASは軽量な転送セッションだけを最大6件まとめて発行し、画像本体はブラウザからGoogle Driveへ最大3件ずつ直接並列転送する。1〜2枚、上書き編集、高速経路で失敗した個別画像は従来の安定経路を使う。画像データをbase64化してまとめてGASへ送る旧方式は使わない。
