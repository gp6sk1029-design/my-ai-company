...
| ⑪ | ローカルミラー同期（**2026-07-29追加・省略禁止**） | `python3 blog/scripts/sync_posts_to_local.py` → `blog/posts/` を最新の本番と一致させる。**ここを飛ばすと手元の写しが古くなり、次の横断点検で誤った判断をする**（実際に原稿9本／公開13記事まで乖離した） |
