...
- 2026/07/19：**記事めしPWA「つくるもの」に🛍️商品説明（概要カード）テンプレを追加（PWAのみ・GAS改修なし）**。製品紹介グループの先頭に配置＝商品写真＋商品名＋キャッチ説明(1〜2行)＋推しポイント3つ(✔)の"つかみ"カード。触った6箇所は全て `blog/pwa-cloudflare/`：①index.htmlの`ai-template-select`（＝選択肢の唯一のマスター。モーダル/バナー/ラベルはここのinnerHTMLを複製するので1箇所追加で全波及）②app.js TEMPLATE_TO_ROLE（`product_desc:'section'`）③TEMPLATE_FIELDS（入力欄ラベル）④buildResearchPrompt（リサーチ補助）⑤AI_TEMPLATES（画像生成プロンプト・末尾は`${COMMON_GUARDS}`）⑥TEMPLATE_PLAN（rank39でスペック表より前・キャプション）。検証：`node --check`＋dev server(:8792)で実機E2E（選択肢表示・欄ラベル切替・生成プロンプトに入力値差込・console 0エラー）→`wrangler pages deploy . --project-name=blog-capture`で本番反映。**学び①：記事めしのテンプレ追加はこの6箇所が定石（役割=既存グループと同じにすればGAS/article_from_meshi.py改修は不要）**。**学び②：本番curl検証は`/index.html`が308で`/`へ飛ぶため0バイトになる→`curl -sL`でルート`/`を見る（app.jsは直取得でOK）**。**運用メモ：wrangler OAuthは期限切れになることがあり、その時はユーザーに`wrangler login`（ブラウザでAllow）を依頼→再デプロイ**。
