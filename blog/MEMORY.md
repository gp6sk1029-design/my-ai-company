...
- 2026/06/13：**編集バナーの比較ギャラリーが都度更新されない不具合を修正**。原因＝比較ギャラリーはupdateBannerPromptSummaryの全再描画(structureKey変化時)でしか描かれず、製品番号変更や画像入替では更新されなかった。修正＝`refreshCompareGallery()`を新設し、renderQueue末尾＋インライン製品番号セレクタ変更時に呼ぶ（キューが変わるたびにギャラリーと重複警告を再評価）。**学び：構造キーで全再描画を間引く設計では、頻繁に変わる従属表示（比較ギャラリー等）は別途「データ変更イベント側」から明示更新する**。
