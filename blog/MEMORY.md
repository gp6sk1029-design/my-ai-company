...
### 全社ルール改定の記録
- 2026/04/27（CPO就任時）: **AI操作の優先順位を Claude in Chrome 一択に転換**。global_rules/CLAUDE_global.md と CLAUDE.md（プロジェクト）に追記。理由：API利用料削減＋学習一貫性＋デバッグ容易性＋OAuth再利用。実証済みの成功事例として「JIN:R吹き出しスロット10枚一括切替（手動15分→自動2分）」がある
